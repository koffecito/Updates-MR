...
func fetchPlayStoreInfo(appID string) (version string, appName string, whatsNew string, err error) {
	...
	if len(nameMatch) > 1 {
		...
		if len(titleMatch) > 1 {
			appName = html.UnescapeString(titleMatch[1])
			appName = strings.TrimSuffix(appName, " - Apps on Google Play")
			appName = strings.TrimSuffix(appName, " – Apps on Google Play")
		}
	}
	...
	return version, appName, whats, nil
}
